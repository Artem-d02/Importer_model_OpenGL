...
int main()
{
	...
	// glfw: создание окна
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Model view window", NULL, NULL);
	if (window == NULL)
	{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Прожектор
		ourShader.setVec3("spotLight.position", campos);
		auto camfront = camera.GetFront();
		ourShader.setVec3("spotLight.direction", camfront);
		ourShader.setVec3("spotLight.ambient", 0.0f, 0.0f, 0.0f);
		ourShader.setVec3("spotLight.diffuse", is_lamp_on, is_lamp_on, is_lamp_on);
		ourShader.setVec3("spotLight.specular", is_lamp_on, is_lamp_on, is_lamp_on);
